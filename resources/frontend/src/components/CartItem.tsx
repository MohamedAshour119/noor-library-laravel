...
export default function CartItem(props: Props) {
    ...
    return (
        <div className={`flex items-center border-b dark:border-dark_border_color py-4 ${is_last_book && add_to_cart_count > 4 ? 'pb-16' : ''}`}>
            <img src={cover} alt="Cover" className="w-24 h-32 ltr:mr-4 rtl:ml-4"/>
            <div className="flex-1">
                <h3 className="text-lg font-bold">{title}</h3>
                <p className="text-gray-500 dark:text-dark_text_color/60">{translation.author}: {author}</p>
                <div className={`flex justify-between items-center`}>
                    <p className="text-gray-500 dark:text-dark_text_color/60">{translation.quantity}: {quantity}</p>
                    <div className="relative flex items-center justify-end w-28 text-text_color">
                        <button
                            ...
                            className="relative hover:bg-white dark:hover:bg-dark_border_color transition-all border dark:text-dark_text_color border-table_border dark:border-dark_border_color rounded-s-lg p-3 h-4 focus:outline-0"
                        >
                        ...
                        <input type="number"
                               ...
                               className="bg-transparent border-x-0 border-y dark:text-dark_text_color border-table_border dark:border-dark_border_color h-4 py-3 text-center text-sm block w-10 focus:outline-0"
                               required
                               value={quantity}
                        />
                        <button
                            ...
                            className="relative hover:bg-white dark:hover:bg-dark_border_color transition-all border dark:text-dark_text_color border-table_border dark:border-dark_border_color rounded-e-lg p-3 h-4 focus:outline-0"
                        >
                            <svg className="w-3 h-3 absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2" aria-hidden="true"
                ...
                <div className="flex justify-between items-center">
                    ...
                    <button
                        onClick={removeItem}
                        className="bg-red-500 hover:bg-red-700 text-white dark:text-dark_text_color font-bold py-2 px-4 mt-2 rounded">
                        {translation.remove}
                    </button>
                </div>
            </div>
