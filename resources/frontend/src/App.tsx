...
function App() {
    const isSearchModalOpenSlice = useSelector(
        (state: RootState) => state.isSearchModalOpenReducer.is_open
    );
    const isUnauthorizedMessageOpenSlice = useSelector(
        (state: RootState) => state.isUnauthorizedMessageOpenReducer.is_open
    );
    ...
    const getTranslation = (namespace: string) => {
        ...
        if (namespaceMapping[decodedNamespace]) {
            if (
                namespaceMapping[decodedNamespace] === "sign-up" ||
                namespaceMapping[decodedNamespace] === "sign-in"
            ) {
                return "Auth";
    ...
    useEffect(() => {
        ...
        if (
            ...
            dynamicRoutePatterns[pathSegments[0]]
        ) {
            namespace =
                dynamicRoutePatterns[pathSegments[0]] +
    ...
    useEffect(() => {
        ...
        const expiration_date = expires_date
            ? new Date(expires_date).getTime()
            : 0;
    ...
    return (
        <>
            {(isSearchModalOpenSlice || isUnauthorizedMessageOpenSlice) && (
                <div
            ...
            />
            <div
                className={`relative dark:bg-dark_main_color ${
                    isAddToCartSidebarSlice ? "max-h-svh overflow-hidden" : ""
                }`}
            >
                <AddToCartSidebar />
                {/* <ChatSupport/> */}
                <Suspense fallback={<div>Loading...</div>}>
                    ...
                    <Routes>
                        ...
                        <Route
                            path="/search-books-results"
                            element={<SearchBookResults />}
                        />
