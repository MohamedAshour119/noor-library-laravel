...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children, header='', ref, parent_styles }) => {
    ...
    return createPortal(
        ...
        >
            <div
                className={`bg-white rounded-lg shadow-xl w-[40rem] transform transition-all duration-300 ease-in-out ${
            ...
            >
                <header className={`flex items-center justify-between p-4 border-b`}>
                    <h3 className="text-red-600 text-xl font-medium">{header}</h3>
                    <div
                        onClick={onClose}
                        className={`bg-main_bg p-2 rounded-lg cursor-pointer`}
                    >
                        <MdClose/>
                    </div>
                </header>
